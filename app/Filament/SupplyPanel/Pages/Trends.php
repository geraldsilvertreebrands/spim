...
class Trends extends Page
{
    ...
    public bool $showRevenueAsTable = false;

    /**
     * Toggle between chart and table view for revenue trend.
     */
    public function toggleRevenueView(): void
    {
        $this->showRevenueAsTable = ! $this->showRevenueAsTable;

        // If switching back to chart view, dispatch event to re-initialize chart
        if (! $this->showRevenueAsTable) {
    ...
    /**
     * Get revenue chart data formatted for table display.
     *
     * @return array<int, array{month: string, revenue: float}>
     */
    public function getRevenueTableData(): array
    {
        if (empty($this->revenueChartData['labels']) || empty($this->revenueChartData['datasets'])) {
            return [];
        }

        $data = [];
        $labels = $this->revenueChartData['labels'];
        $values = $this->revenueChartData['datasets'][0]['data'] ?? [];
